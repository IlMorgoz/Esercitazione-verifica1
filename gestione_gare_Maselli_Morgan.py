...
def inserisci_dati():
    ...
    for piatto in piatti:
        while(True):
            creatività=int(input("Inserisci il piunteggio per la creatività") in range(1,11))
            if(controllo(categoria)): break
        while(True):
            gusto=int(input("Inserisci il piunteggio per il gusto") in range(1,11))
            if(controllo(gusto)): break
        while(True):
            originalità=int(input("Inserisci il piunteggio per l'originalità") in range(1,11))
...
def inserisci_nuovo_chef(gare,nominativo,piatto,risultati,categoria):
    aggiunta={nominativo:[]}
    for i in range(0,5):
        aggiunta[nominativo].append(piatto,risultati[i],categoria)
    gare.update(aggiunta)